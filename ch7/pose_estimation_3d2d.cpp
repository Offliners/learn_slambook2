                          ...
                          std::vector<DMatch> &matches)
{
    ...
    //-- 第三步:對两幅圖像中的BRIEF描述子進行匹配，使用 Hamming 距離
    vector<DMatch> match;
    ...
    //找出所有匹配之間的最小距離和最大距離, 即是最相似的和最不相似的两组點之間的距離
    ...
    //當描述子之間的距离大於两倍的最小距離時,即認為匹配有誤.但有時候最小距離會非常小,設置一個檢驗值30作為下限.
    for(int i = 0; i < descriptors_1.rows; ++i)
    ...
    Mat &R, Mat &t)
{
    ...
    optimizer.setVerbose(true);       // 打開調試输出
